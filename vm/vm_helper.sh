...
handle-vm-try-with-iso() {
    ...
    run-command "vm_try --hd \"$hdd\" --iso \"$iso\"$uefi_flag"
}
...
handle-vm-try-without-iso() {
    ...
    run-command "vm_try --hd \"$hdd\"$uefi_flag"
}
...
handle-vm-clone() {
    ...
    run-command "vm_clone --iso \"$iso\" --src \"$src_image\" --dst \"$dst_image\"$uefi_flag"
}
...
handle-ventoy-usb-test() { ( sudo ventoy_usb_test ); }
handle-vm-create-disk() { ( vm_create_disk ); }
handle-vm-disk-manager() { ( sudo vm_disk_manager ); }
